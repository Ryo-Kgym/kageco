export const paths = {
  ...
  household: {
    ...
    creditCardDetail: ({ id }: { id: string }) => `/household/detail/creditCard/${id}/edit` as "/",
    ...
    categoryRanking: ({ date }: { date: Date }) =>
      `/household/detailList/categoryRanking/${date.toISOString().slice(0, 10)}` as "/",
    rankCategoryYearly: ({ date }: { date: Date }) =>
      `/household/rank/category/yearly/${date.toISOString().slice(0, 10)}` as "/",
    detailListByCategory: ({
      ...
      year: Date;
      categoryId: string;
    }) => `/household/rank/category/yearly/${year.toISOString().slice(0, 10)}/${categoryId}` as "/",
    setting: {
      root: "/household/setting" as "/",
      dashboard: "/household/setting/dashboard" as "/",
      dashboardEdit: ({ id }: { id: string }) => `/household/setting/dashboard/edit/${id}` as "/",
